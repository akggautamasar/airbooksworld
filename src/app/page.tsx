  ...
  return (
    <div className="grain min-h-screen overflow-hidden">
      <section className="library-hero mx-auto max-w-[1500px] px-5 text-center">
        <p className="font-mono text-[9px] uppercase tracking-[.42em] text-[#8d847a]">
          A personal archive
        </p>
        ...
        <p className="mt-28 font-mono text-[9px] uppercase tracking-[.38em] text-[#927f6d] sm:mt-8">
          {total || "—"} volumes
        </p>
        ...
        <div className="mt-10 flex justify-center sm:mt-6">
          <Link
            href="/upload"
            className="rounded-full border border-[#c5bdb3] bg-[#eeeae4]/55 px-9 py-3 font-mono text-[9px] uppercase tracking-[.27em] text-[#5f574e] shadow-sm transition hover:border-[#8e8478] hover:bg-[#f7f4ef]"
          >
            Upload a book
          </Link>
        </div>
            ...
            <div className="flex min-w-max gap-2 px-1">
              {["All", ...tags.slice(0, 14)].map((t) => {
                ...
                return (
                  <Link
                    key={t}
                    href={t === "All" ? "/" : `/?tag=${encodeURIComponent(t)}`}
                    className={`rounded-full border px-4 py-1.5 font-mono text-[9px] uppercase tracking-[.18em] transition ${
                      active
                        ? "border-[#7e756b] bg-[#ded8d0] text-[#403a34]"
                        : "border-[#c8c0b7] text-[#81786e] hover:border-[#93897e] hover:text-[#4c453e]"
                    }`}
                  >
                    {t}
                  </Link>
      ...
      {error ? (
        ...
        <section className="library-shelf mt-14 sm:mt-6">
          <Shelf books={books} />
        </section>
      ) : (
        ...
        <nav className="flex items-center justify-center gap-7 pb-14 pt-8 sm:pt-4">
          <Link
            ...
            className={`rounded-full border p-3 ${
              page <= 1
                ? "pointer-events-none border-[#ddd7cf] text-[#c8c0b7]"
                : "border-[#c1b8ae] text-[#5f574e] hover:bg-white/50"
            }`}
          >
            <ChevronLeft className="h-4 w-4" />
          </Link>
          <span className="font-mono text-[9px] uppercase tracking-[.22em] text-[#81786e]">
