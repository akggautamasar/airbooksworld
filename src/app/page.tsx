...
export default async function HomePage({searchParams}:Props){
  ...
  const page=Math.max(1,parseInt(searchParams.page||"1",10)||1),offset=(page-1)*PAGE_SIZE;
  let books:Awaited<ReturnType<typeof fetchBooks>>["books"]=[],total=0,tags:string[]=[],error:string|null=null;
  try{const [booksRes,tagsRes]=await Promise.all([fetchBooks({q,tag,author,limit:PAGE_SIZE,offset}),fetchTags()]);books=booksRes.books;total=booksRes.total;tags=tagsRes.filter(item=>!isRecommendationTag(item))}catch(e:any){error=e?.message||"Failed to load books. Is the backend running?"}
  ...
  return <div className="grain min-h-screen overflow-hidden bg-transparent">
    ...
    {error?<div className="mx-auto mt-20 max-w-xl rounded-2xl border border-[#cfc3b6] bg-[#FAF8F5]/80 p-8 text-center"><p className="font-display text-2xl text-[#4a4139]">The shelves are quiet.</p><p className="mt-2 font-sans text-sm text-[#81776c]">{error}</p></div>:books.length>0?<section className="library-shelf mt-10 sm:mt-8"><Shelf books={books}/></section>:<div className="mx-auto mt-20 max-w-md text-center"><p className="font-display text-3xl text-[#514940]">No books match.</p><p className="mt-2 font-sans text-sm text-[#8b8278]">Try another title, author or tag.</p></div>}

    {!error&&totalPages>1&&<nav className="flex items-center justify-center gap-7 pb-8 pt-8"><Link href={pageHref(Math.max(1,page-1))} aria-disabled={page<=1} className={`rounded-full border p-3 ${page<=1?"pointer-events-none border-[#ddd7cf] text-[#c8c0b7]":"border-[#c1b8ae] text-[#5f574e] hover:bg-white/50"}`}><ChevronLeft className="h-4 w-4"/></Link><span className="font-mono text-[9px] uppercase tracking-[.22em] text-[#81786e]">Page {page} of {totalPages}</span><Link href={pageHref(Math.min(totalPages,page+1))} aria-disabled={page>=totalPages} className={`rounded-full border p-3 ${page>=totalPages?"pointer-events-none border-[#ddd7cf] text-[#c8c0b7]":"border-[#c1b8ae] text-[#5f574e] hover:bg-white/50"}`}><ChevronRight className="h-4 w-4"/></Link></nav>}
