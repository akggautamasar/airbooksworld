...
export function Shelf({ books }:{books:Book[]}) {
  ...
  // Match the Carollia reference: books are flat toward the viewer in the
  // middle and progressively turn away toward the edges while scrolling.
  useEffect(() => {
    const el = rail.current;
    if (!el) return;
    const update = () => {
  ...
  return <>
    <div className="relative mx-auto my-6 w-full max-w-6xl px-2 sm:px-4">
      <div className="shelf-stage-3d shelf-edge-mask w-full overflow-hidden">
        <div
          ref={rail}
          className={`shelf-rail carollia-rail no-scrollbar flex cursor-grab items-end overflow-x-auto select-none ${drag ? "cursor-grabbing" : ""}`}
          onWheel={wheel}
          onScroll={wrap}
          onPointerDown={e => {
            if (e.button !== 0) return;
            setDrag({x:e.clientX,scroll:e.currentTarget.scrollLeft});
        ...
        >
          <div className="shelf-row carollia-rail-row">
            {items.map((book,i) => (
              <BookSpine key={`${book.id}-${i}`} book={book} onOpen={(b,rect) => setOpen({book:b,rect})}/>
            ))}
          </div>
      ...
      <div className="relative mt-[-1px] w-full px-6 pointer-events-none">
        <div className="h-px w-full bg-gradient-to-r from-transparent via-[#C5BDAF]/70 to-transparent"/>
        <div className="h-8 w-full bg-gradient-to-b from-black/[0.07] via-black/[0.02] to-transparent blur-[3px]"/>
      </div>
    </div>
