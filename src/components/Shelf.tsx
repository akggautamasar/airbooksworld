...
export function Shelf({ books }: { books: Book[] }) {
  ...
  function move(e: React.PointerEvent<HTMLDivElement>) {
    ...
    e.currentTarget.scrollLeft = drag.scroll - (e.clientX - drag.x);
  }
  ...
  return (
    <>
      <div ref={rail} className="shelf-rail no-scrollbar" onWheel={wheel} onScroll={wrap} onPointerDown={e=>setDrag({x:e.clientX,scroll:e.currentTarget.scrollLeft})} onPointerMove={move} onPointerUp={()=>setDrag(null)} onPointerCancel={()=>setDrag(null)}>
...
function BookDetail({book,rect,onClose}:{book:Book;rect:Rect;onClose:()=>void}) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#24211d]/25 backdrop-blur-md transition-opacity duration-500" onClick={onClose}>
      <div className="absolute inset-0 flex items-center justify-center p-5 sm:p-10" onClick={e=>e.stopPropagation()}>
        <button aria-label="Close" onClick={onClose} className="absolute right-5 top-5 rounded-full border border-white/60 bg-[#f7f3ed]/80 p-2 text-[#4d453c] hover:bg-white"><X className="h-5 w-5" /></button>
        <div className={`detail-card w-full max-w-4xl grid md:grid-cols-[minmax(210px,330px)_1fr] gap-8 md:gap-12 items-center ${ready?'detail-open':''}`} style={{'--origin-x':`${rect.left+rect.width/2}px`,'--origin-y':`${rect.top+rect.height/2}px`} as React.CSSProperties}>
          <div className="detail-cover-wrap">
            {cover ? <img src={cover} alt={`Cover of ${book.title}`} onError={()=>setFailed(true)} className="detail-cover" /> : <div className="detail-cover detail-fallback"><span>{book.title}</span><small>{book.author}</small></div>}
          </div>
          <div className="rounded-2xl border border-[#d4cbc0] bg-[#f7f3ed]/95 p-7 sm:p-10 shadow-[0_30px_90px_rgba(34,29,24,.18)]">
            <div className="font-mono text-[9px] uppercase tracking-[.22em] text-[#93897d]">AIRBOOKS · {book.filename.split('.').pop()?.toUpperCase()}</div>
            <h2 className="mt-4 font-display text-4xl sm:text-5xl leading-[.95] text-[#29241f]">{book.title}</h2>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={`/book/${book.id}`} onClick={onClose} className="rounded-full bg-[#29241f] px-5 py-2.5 font-mono text-[10px] uppercase tracking-[.16em] text-[#f7f3ed]">Open details</Link>
              <Link href={`/book/${book.id}/read`} onClick={onClose} className="rounded-full border border-[#bdb3a8] px-5 py-2.5 font-mono text-[10px] uppercase tracking-[.16em] text-[#4f473f]">Read</Link>
              <a href={getDownloadUrl(book.id)} onClick={onClose} download className="inline-flex items-center gap-2 rounded-full border border-[#bdb3a8] px-4 py-2.5 font-mono text-[10px] uppercase tracking-[.16em] text-[#4f473f]"><Download className="h-3.5 w-3.5"/>Download</a>
            </div>
          </div>
        </div>
      </div>
