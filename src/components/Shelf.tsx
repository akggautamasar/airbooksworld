...
export function Shelf({books}:{books:Book[]}){
  ...
  useEffect(()=>{const el=rail.current;if(!el||copies!==3)return;requestAnimationFrame(()=>{el.scrollLeft=el.scrollWidth/3})},[books.length,copies]);

  useEffect(()=>{
    const el=rail.current;if(!el)return;
    const update=()=>{const rr=el.getBoundingClientRect(),center=rr.left+rr.width/2,half=Math.max(1,rr.width*.48);el.querySelectorAll<HTMLElement>(".book-spine-hit").forEach(node=>{const r=node.getBoundingClientRect(),c=r.left+r.width/2,n=Math.max(-1,Math.min(1,(c-center)/half)),e=Math.sign(n)*Math.pow(Math.abs(n),1.18);node.style.setProperty("--swivel-y",`${(-e*34).toFixed(2)}deg`);node.style.setProperty("--swivel-z",`${((1-Math.abs(n))*28).toFixed(1)}px`);node.style.setProperty("--swivel-scale",(1+(1-Math.abs(n))*.025).toFixed(3))})};
    let frame=0;const schedule=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(update)};update();el.addEventListener("scroll",schedule,{passive:true});window.addEventListener("resize",schedule,{passive:true});return()=>{cancelAnimationFrame(frame);el.removeEventListener("scroll",schedule);window.removeEventListener("resize",schedule)};
  },[items.length]);

  function wheel(e:React.WheelEvent<HTMLDivElement>){if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){e.preventDefault();e.currentTarget.scrollLeft+=e.deltaY}}
  function move(e:React.PointerEvent<HTMLDivElement>){if(!drag)return;e.currentTarget.setPointerCapture(e.pointerId);e.currentTarget.scrollLeft=drag.scroll-(e.clientX-drag.x)}
  function wrap(){const el=rail.current;if(!el||copies!==3)return;const segment=el.scrollWidth/3;if(el.scrollLeft<segment*.45)el.scrollLeft+=segment;if(el.scrollLeft>segment*1.55)el.scrollLeft-=segment}
  function scrollBy(amount:number){rail.current?.scrollBy({left:amount,behavior:"smooth"})}

  return <>
    <div className="relative mx-auto w-full max-w-6xl px-2 sm:px-4">
      <button aria-label="Scroll shelf left" onClick={()=>scrollBy(-320)} className="carollia-shelf-nav absolute left-1 sm:left-3 top-1/2 z-30 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full text-[#241F19] transition hover:scale-105"><ChevronLeft className="h-4 w-4"/></button>
      <button aria-label="Scroll shelf right" onClick={()=>scrollBy(320)} className="carollia-shelf-nav absolute right-1 sm:right-3 top-1/2 z-30 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full text-[#241F19] transition hover:scale-105"><ChevronRight className="h-4 w-4"/></button>
      <div className="shelf-stage-3d shelf-edge-mask w-full overflow-hidden">
        <div ref={rail} className="shelf-rail carollia-rail no-scrollbar" onWheel={wheel} onScroll={wrap} onPointerDown={e=>setDrag({x:e.clientX,scroll:e.currentTarget.scrollLeft})} onPointerMove={move} onPointerUp={()=>setDrag(null)} onPointerCancel={()=>setDrag(null)}>
...
function BookDetail({book,rect,onClose}:{book:Book;rect:Rect;onClose:()=>void}){
  ...
  return <div className="fixed inset-0 z-[200] bg-[#24211d]/65 backdrop-blur-xl transition-opacity duration-500" onClick={onClose}>
    <div className="absolute inset-0 flex items-center justify-center p-4 sm:p-8" onClick={e=>e.stopPropagation()}>
      ...
      <div className={`detail-card w-full max-w-4xl grid md:grid-cols-[minmax(210px,330px)_1fr] gap-8 md:gap-12 items-center ${ready?"detail-open":""}`} style={{"--origin-x":`${rect.left+rect.width/2}px`,"--origin-y":`${rect.top+rect.height/2}px`} as React.CSSProperties}>
        ...
        <div className="rounded-2xl border border-[#d4cbc0] bg-[#FAF8F5]/95 p-7 sm:p-10 shadow-[0_30px_90px_rgba(34,29,24,.18)]">
          ...
          {book.author&&<p className="mt-3 font-sans text-lg text-[#70675d]">{book.author}</p>}
          {book.description&&<p className="mt-6 max-h-40 overflow-auto font-sans text-[15px] leading-7 text-[#625a51]">{book.description}</p>}
