    ...
    <main className={`book-experience min-h-screen bg-[#F5F2EB] text-[#241F19] ${closing ? "is-closing" : ""}`}>
      <div className="book-experience-inner mx-auto flex min-h-screen w-full max-w-5xl flex-col px-6 pb-12 pt-24 sm:px-10 sm:pt-28 lg:px-12">
        ...
        <div className="book-experience-actions mt-10 border-t border-[#d9d2c7] pt-7">
          ...
          <div className="mt-8 flex items-center justify-between gap-4 font-mono text-[10px] uppercase tracking-[.2em] text-[#6f665c]"><div className="flex items-center gap-5">{previousId ? <Link href={`/book/${previousId}`} onClick={() => setOriginFor(previousId)} className="inline-flex items-center gap-2 transition-colors hover:text-[#9a6c55]"><ArrowLeft className="h-4 w-4" /> Previous</Link> : <span className="opacity-30">Previous</span>}{nextId ? <Link href={`/book/${nextId}`} onClick={() => setOriginFor(nextId)} className="inline-flex items-center gap-2 transition-colors hover:text-[#9a6c55]">Next <ArrowRight className="h-4 w-4" /></Link> : <span className="opacity-30">Next</span>}</div><button type="button" onClick={shelveIt} disabled={closing} className="inline-flex items-center gap-2 text-[#8f634e] transition-all hover:-translate-y-0.5 disabled:opacity-50"><Bookmark className="h-4 w-4" /> {returning ? "Shelving…" : "Shelve it"}</button></div>
        </div>
      </div>
    </main>
  );
