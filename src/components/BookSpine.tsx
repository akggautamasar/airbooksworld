...
export function BookSpine({book,onOpen}:Props){
  ...
  return <button ref={ref} type="button" aria-label={`Open ${book.title}`} className="book-spine-hit carollia-book-hit shrink-0 self-end relative" style={style} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={()=>setHovered(true)} onBlur={()=>setHovered(false)} onClick={open}>
    <span className="carollia-book-body book-spine-body absolute inset-0 block" style={{transform:"translateZ(0) scale(1) rotateY(0deg) rotateZ(0deg)",zIndex:hovered?40:1}}>
      ...
      {cover&&<span className="carollia-front-cover absolute bottom-0 left-full z-10 block overflow-hidden rounded-r-[2px] shadow-2xl" style={{width:178,height:"100%",transformOrigin:"left center",transform:"rotateY(var(--cover-angle, 90deg))",transition:"transform 280ms cubic-bezier(.22,.61,.36,1)",backgroundColor:spine}}><img src={cover} alt="" className="h-full w-full object-cover"/></span>}
    </span>
