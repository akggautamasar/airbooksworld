...
export function BookSpine({book,onOpen}:Props){
  ...
  return <button ref={ref} type="button" aria-label={`Open ${book.title}`} className="book-spine-hit carollia-book-hit shrink-0 self-end relative" style={style} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={()=>setHovered(true)} onBlur={()=>setHovered(false)} onClick={open}>
    <span className="carollia-book-body book-spine-body absolute inset-0 block" style={{transform:"translateZ(var(--swivel-z, 0px)) scale(var(--swivel-scale, 1)) rotateY(var(--swivel-y, 0deg)) rotateZ(var(--book-rotation, 0deg))",zIndex:hovered?40:1}}>
      ...
      {cover&&<span className="carollia-front-cover absolute bottom-0 left-full top-0 z-10 block overflow-hidden rounded-r-[2px] shadow-2xl" style={{width:178,transformOrigin:"left center",transform:"rotateY(90deg)",backgroundColor:spine}}><img src={cover} alt="" className="h-full w-full object-cover"/></span>}
    </span>
