...
export function BookSpine({book,onOpen,rotateY=0}:Props){
  ...
  return <button ref={ref} type="button" data-book-id={book.id} aria-label={`Open ${book.title}`} className="book-spine-hit carollia-book-hit shrink-0 self-end relative" style={style} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={()=>setHovered(true)} onBlur={()=>setHovered(false)} onClick={open}>
    <span className="carollia-book-body book-spine-body absolute inset-0 block" style={{transform:`translateZ(${pull + 5}px) translateY(${lift}px) rotateY(${rotateY}deg) rotateZ(${lean}deg)`,zIndex:hovered?40:1,transition:"transform 360ms cubic-bezier(.22,1,.32,1), box-shadow 360ms ease",transformStyle:"preserve-3d"}}>
      <span className="carollia-book-cover book-spine-cover relative block h-full w-full overflow-hidden rounded-[1px]" style={{background:`linear-gradient(105deg,rgba(255,255,255,.20),transparent 18%,rgba(0,0,0,.16) 84%,rgba(0,0,0,.36)),linear-gradient(90deg,${variant.light},${spine} 45%,${variant.dark})`}}>
        ...
        <span className="absolute -top-[6px] left-0 right-0 z-30 h-[7px] rounded-t-[1px] border-t border-[#c5beaf] bg-[#ede8de]" style={{backgroundImage:"repeating-linear-gradient(90deg,#d8d0c0,#d8d0c0 1px,#f4f0e6 1px,#f4f0e6 3px)",boxShadow:"inset 0 1px 2px rgba(0,0,0,.2)",transform:"rotateX(78deg)",transformOrigin:"bottom center"}}/>
      ...
      {cover&&<span className="carollia-front-cover absolute bottom-0 left-full z-10 block overflow-hidden rounded-r-[2px]" style={{width:148,height:"100%",transformOrigin:"left center",transform:`rotateY(${hovered?"48deg":"var(--cover-angle, 90deg)"})`,transition:"transform 420ms cubic-bezier(.22,1,.36,1), box-shadow 420ms ease",backgroundColor:spine,boxShadow:hovered?"14px 8px 22px rgba(0,0,0,.22)":"8px 6px 18px rgba(0,0,0,.18)"}}><img src={cover} alt="" className="h-full w-full object-cover"/></span>}
    </span>
    <span className="carollia-book-shadow pointer-events-none absolute -bottom-[6px] left-0 right-0 h-2 rounded-full bg-black/55 blur-[2px] transition-all duration-300" style={{opacity:hovered?.18:.78,transform:hovered?"scaleX(.78)":"scaleX(1)"}}/>
