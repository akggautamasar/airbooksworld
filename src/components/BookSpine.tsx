...
export function BookSpine({book,onOpen,rotateY=0}:Props){
  ...
  const style={"--book-w":`${width}px`,"--book-h":`${height}px`,"--book-depth":"4px","--book-rotation":"0deg`,touchAction:"manipulation","--cover-w":`${Math.round(Math.max(105,Math.min(150,height*.62)))}px`} as CSSProperties;
  ...
  const desktopCoverWidth=Math.round(Math.max(105,Math.min(150,height*.62)));
  ...
  return <button ref={ref} type="button" data-book-id={book.id} aria-label={`Open ${book.title}`} className="book-spine-hit carollia-book-hit shrink-0 self-end relative" style={style} onPointerDown={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={()=>setHovered(true)} onBlur={()=>setHovered(false)} onClick={open}>
    ...
    <span className="carollia-book-body book-spine-body absolute inset-0 block" style={{transform:`translateZ(${pull + 4}px) translateY(${lift}px) rotateY(${rotateY}deg) rotateZ(${lean}deg)`,zIndex:hovered?40:1,transition:"transform 360ms cubic-bezier(.22,1,.32,1), box-shadow 360ms ease",transformStyle:"preserve-3d"}}>
      ...
      {cover&&<span className="carollia-front-cover absolute bottom-0 left-full z-10 block overflow-hidden rounded-r-[2px]" style={{width:showFullCover?desktopCoverWidth:30,height:"100%",transformOrigin:"left center",transform:`rotateY(${showFullCover?"0deg":"90deg"})`,transition:"transform 520ms cubic-bezier(.16,1,.3,1), width 520ms cubic-bezier(.16,1,.3,1), box-shadow 520ms ease",backgroundColor:spine,boxShadow:showFullCover?"16px 8px 22px rgba(0,0,0,.24)":"5px 4px 12px rgba(0,0,0,.15)"}}><img src={cover} alt="" className="h-full w-full object-cover"/></span>}
    </span>
