...
export function BookSpine({book,onOpen,rotateY=0}:Props){
  ...
  return <button ref={ref} type="button" aria-label={`Open ${book.title}`} className="book-spine-hit carollia-book-hit shrink-0 self-end relative" style={style} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} onFocus={()=>setHovered(true)} onBlur={()=>setHovered(false)} onClick={open}>
    <span className="carollia-book-body book-spine-body absolute inset-0 block" style={{transform:`translateZ(${pull + 5}px) translateY(${lift}px) rotateY(${rotateY}deg) rotateZ(var(--book-rotation, 0deg))`,zIndex:hovered?40:1,transition:"transform 300ms cubic-bezier(.22,1,.32,1), box-shadow 300ms ease",transformStyle:"preserve-3d"}}>
