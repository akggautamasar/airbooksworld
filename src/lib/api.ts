...
export type GenerateAllCoversResult = { total_missing: number; generated: number; skipped: number; failed: string[] };
export async function adminGenerateAllCovers(password: string): Promise<GenerateAllCoversResult> { const res = await fetch(`${getBase()}/api/books/covers/generate-all`, { method: "POST", headers: { "X-Admin-Password": password } }); if (!res.ok) { if (res.status === 401) throw new Error("Invalid admin password"); throw new Error(`Failed to generate covers: ${res.status}`); } return res.json(); }
export type DuplicateGroup = { method: "identical_file" | "same_name_and_size"; books: (Book & { suggested_keep: boolean })[] };
export type DuplicatesResponse = { status: string; group_count: number; duplicate_book_count: number; groups: DuplicateGroup[] };
export async function fetchDuplicateGroups(password: string): Promise<DuplicatesResponse> { const res = await fetch(`${getBase()}/api/books/admin/duplicates`, { headers: { "X-Admin-Password": password } }); if (!res.ok) { if (res.status === 401) throw new Error("Invalid admin password"); throw new Error(`Failed to fetch duplicates: ${res.status}`); } return res.json(); }
export type ImportStatus = "validating" | "scanning" | "fetching" | "deduplicating" | "importing" | "enriching" | "done" | "cancelled" | "error";
export type ImportProgress = { import_id: string; status: ImportStatus; channel?: string; channel_name?: string; imported: number; skipped: number; skipped_duplicate: number; skipped_not_book: number; skipped_unreadable?: number; import_method?: "scan-first" | "forward-first" | null; read_access?: boolean | null; errors: number; fetched: number; total_scan: number; total_media: number; batches_done: number; enriched: number; covers: number; enrich_total: number; enrich_done?: number; skipped_large?: number; skipped_memory?: number; metadata_updated?: number; elapsed?: number; error_msg?: string | null };
export type StartImportOptions = { channel: string; start_msg_id?: number | null; end_msg_id?: number | null; skip_duplicates?: boolean; enrich?: boolean; generate_covers?: boolean };
function adminHeaders(password: string) { return { "Content-Type": "application/json", "X-Admin-Password": password }; }
async function adminError(res: Response, fallback: string): Promise<never> { if (res.status === 401) throw new Error("Invalid admin password"); let detail = ""; try { const body = await res.json(); detail = body?.detail || ""; } catch {} throw new Error(detail || `${fallback} (${res.status})`); }
export async function startChannelImport(password: string, options: StartImportOptions) { const res = await fetch(`${getBase()}/api/books/admin/import`, { method: "POST", headers: adminHeaders(password), body: JSON.stringify(options) }); if (!res.ok) await adminError(res, "Failed to start import"); return res.json(); }
export async function fetchImportProgress(password: string, importId: string): Promise<ImportProgress> { const res = await fetch(`${getBase()}/api/books/admin/import/${encodeURIComponent(importId)}`, { headers: { "X-Admin-Password": password }, cache: "no-store" }); if (!res.ok) await adminError(res, "Failed to fetch import progress"); return (await res.json()).data; }
export async function cancelChannelImport(password: string, importId: string) { const res = await fetch(`${getBase()}/api/books/admin/import/${encodeURIComponent(importId)}/cancel`, { method: "POST", headers: { "X-Admin-Password": password } }); if (!res.ok) await adminError(res, "Failed to cancel import"); }
export async function fetchEnrichStatus(password: string) { const res = await fetch(`${getBase()}/api/books/admin/enrich-status`, { headers: { "X-Admin-Password": password }, cache: "no-store" }); if (!res.ok) await adminError(res, "Failed to fetch enrich status"); return res.json(); }
